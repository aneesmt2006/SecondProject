export const TYPES = {
    DoctorService:Symbol.for("DoctorService"),
    DoctorRespository:Symbol.for("DoctorRespository"),
    DoctorProfileService: Symbol.for("DoctorProfileService"),
    DoctorProfileRepository: Symbol.for("DoctorProfileRepository"),
    FetusService: Symbol.for("FetusService"),
    FetusRepository: Symbol.for("FetusRepository"),
    SymptomsService:Symbol.for("SymptomsService"),
    SymptomsRepository:Symbol.for('SymptomsRepository')
}