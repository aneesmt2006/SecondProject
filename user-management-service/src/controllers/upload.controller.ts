...
import { generateReadUrl } from "../utils/s3.read.signedurl.js";
...
export class UplaodController implements interfaces.Controller {
    ...
    @httpGet('/read-signed-url')
    public async readSignedUrl(req:Request,res:Response,next:NextFunction){
        try {
            const {key} = req.query
            const signedUrl = await generateReadUrl(String(key))
            commonResponse.success(res,AUTH_RESPONSE_MESSAGES.SIGNED_URL_SUCCESS,signedUrl,HTTP_STATUS.OK)
        } catch (error) {
            next(error)
        }
    }
