...
import { closeRabbitMQ } from "./config/rabbitmq.config.js";
import { startPaymentConsumer } from "./consumers/payment.consumer.js";
import { startSlotExpiryConsumer } from "./consumers/slot.expiry.consumer.js";

const PORT = config.port || 3004;

// Consumers start now (Connections are already established via top-level await in config files)
try {
...
const server = app.listen(PORT, () => {
  logger.info(`Appointment Service running on port ${PORT} [${config.deploy}]`);
});
...
const shutdown = async (signal: string): Promise<void> => {
  ...
  server.close(async () => {
    ...
    try {
      ...
      logger.error("Error closing RabbitMQ", { error: (err as Error).message });
    }
